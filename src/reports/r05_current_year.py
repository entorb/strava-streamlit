...
def main() -> None:  # noqa: D103, PLR0915
    ...
    sel_types = col2.multiselect(
        label="Sports", options=list_sports(df), key="sel_types"
    )
    if sel_types:
    ...
    cols[2].metric(
        label="Hours", value=f"{round(hour_sum)} h", label_visibility="hidden"
    )
    cols[3].metric(
        label="Distance", value=f"{round(km_sum)} km", label_visibility="hidden"
    )
    cols[4].metric(
        label="Elevation",
        value=f"{round(elev_km_sum, 1)} km",
    ...
    cols[1].metric(
        label="Active Days %",
        value=f"{round(100 * active_days / past_days_in_year, 1)} %",
    )
    cols[2].metric(
